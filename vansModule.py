...
def get_html(driver):
    ...
    #신발 검색 버튼 누르기
    searchbutton = driver.find_element_by_xpath('//*[@id="header-search"]/form/button/svg')
    searchbutton.click()
    #스크롤 기능
    ...
    return driver.page_source #결과의 소스

#각 해당 데이터 파싱
def get_info(html, filepath):
    ...
    df.to_csv(filepath, encoding='utf-8', index=None)
    return df
...
def get_images(driver, page_urls, img_path):
    index = 0
    for page in page_urls:
        ...
        for link in img_url:
            try:
                index += 1
                req.urlretrieve(link, f'{img_path}/{index}.jpg')
            except:
                continue
...
if __name__ == '__main__':
    ...
    print(html)
    get_info(html, "data/vans_data.csv")
    page_urls = get_urls(html)
    print(page_urls[:3])
    get_images(driver, page_urls, './img')
    driver.close()